...
* `deploy_zdm_proxy.yml` to deploy and configure the desired number of ZDM Proxy instances. The ZDM Proxy processes are deployed as Docker containers and the whole setup is automated by the playbook.
...
* `collect_zdm_proxy_logs.yml` to collect the logs from all proxies into a single zip archive.
...
![AWS infrastructure and networking layout provisioned by Terraform](images/ZDM 2.0 Proxy Network Layout.jpeg)
...
* The root module `self-contained-deployment-root-aws`:  which creates the ZDM VPC and its components (yellow area of the diagram) and also peers this VPC with one existing specified VPC. Additional VPCs can be peered to the ZDM VPC manually if necessary.
...
You will be using the `docker compose` command, which in some installations may 
